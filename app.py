...
class App:
    ...
    def init_results_panel(self, menu_btn):
        ...
        def untoogle_other(btn):
            for _, _, _, solution in sorted_buttons:
                button = getattr(solution, "button", None)
                if button and button != btn and button.is_enabled and button.is_toogled:
                    button.is_toogled = False
        ...
        def make_solution_button(solution, number):
            if not hasattr(solution, "button"):
                ...
                solution.button.highlight_tick = 30
                solution.button.changed = partial(toogle_button, btn=solution.button, solution=solution)
